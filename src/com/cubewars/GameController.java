 ...
 * The game controller.
 * 
 * The game controller implements the game rules and actions. It keeps track of the player turns and actions so they are not able to repeat movements in the same turn.
 * 
 * @author pyrosphere3
 */
public class GameController extends Game
{
	 ...
	 * 
	 * {@link #select(Coordinates)} can be used to obtain the class of the current game entity placed in a given cell without exposing the actual GameObject.
	 ...
	 * 
	 * Places the entity placed in origin in the destination and updates its current screen position, provided that the path is not longer than the character's travelling distance.
	 * 
	 * @param source Source {@link Coordinates}
	 * @param destination Destination {@link Coordinates}
	 * @return A {@link Response} telling whether the movement has completed successfully (OK) or not (INVALID).
	...
	public Response move (Coordinates source, Coordinates destination, Player player)
	{
		...
		{
			/*
			 * If our character can travel to the destination position, do it. We'll assume that only objects derived from the Character class can be moved across the board.
			 */
			Character personaje = (Character) map.get (source);
	
			if (Character.class.isAssignableFrom (select (source)) && source.distance (destination) <= personaje.getTravel ())
			{
				System.out.println ("[CNTROL] Moving " + personaje.toString () + ": " + source.toString () + " -> " + destination.toString () + ".");
			...
			{
				System.out.println ("[CNTROL] Cannot move " + personaje.toString () + ".");
	 ...
	 * 
	 * If the objective is inside our attack radius, deal points of damage. This method takes care of character deletion when its healts decreases below 0, and should work with characters and game objects.
	 * 
	 * @param source Attacker's coordinates.
	 * @param destination Objective's coordinates.
	 * @return A {@link Response} telling whether the attack has completed successfully (OK) or not (INVALID).
	 */
	public Response attack (Coordinates source, Coordinates destination, Player player)
	{
		...
		if (turns.canAttack (player))
		{
			...
			{
				System.out.println ("[CNTROL] " + attacker.toString () + " is too far from "
